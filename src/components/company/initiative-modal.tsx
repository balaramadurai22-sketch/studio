...
type InitiativeModalProps = {
  initiative: CompanyInitiative | null;
  isOpen: boolean;
  onClose: () => void;
};

export default function InitiativeModal({ initiative, isOpen, onClose }: InitiativeModalProps) {
  if (!initiative) {
    return null;
  }
      ...
      <DialogContent className="sm:max-w-md bg-background text-foreground border-foreground">
        <DialogHeader>
          <div className="flex items-center gap-4 mb-4">
            ...
            <DialogTitle className="text-2xl font-bold font-headline">{initiative.name}</DialogTitle>
          </div>
        </DialogHeader>
        ...
        <Button asChild onClick={onClose} variant="outline" className="border-foreground hover:bg-foreground hover:text-background">
          <Link href={`/company/${initiative.id}`}>
            View More
